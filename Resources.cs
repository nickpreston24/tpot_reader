...
public static class Resources
{
    ...
    public static void LoadResources(this Assembly? assembly, bool debug = false)
    {
        ...
        if (debug) Console.WriteLine("total resources found: " + resources.Length);
        if (resources.Length == 0)
            return;

        if (debug) Console.WriteLine($"Resources in {assembly.FullName}");
        if (debug)
        ...
        public static string GetFile(string filename, bool debug = false)
        {
            ...
            if (!Path.HasExtension(filename))
                throw new ArgumentException(nameof(filename) +
                                            " must have an extension!"); // do NOT allow extensionless files!
